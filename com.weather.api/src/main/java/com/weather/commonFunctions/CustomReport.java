...
public class CustomReport implements IReporter {
	...
	private static NumberFormat integerFormat = NumberFormat
			.getIntegerInstance();
	private static NumberFormat decimalFormat = NumberFormat
			.getNumberInstance();
	...
	protected void writeReportTitle(String title) {
		writer.print("<center><h1>" + title + " - " + getDateAsString()
				+ "</h1></center>");
	}
	...
	protected void writeSuiteSummary() {
		...
		for (SuiteResult suiteResult : suiteResults) {
			...
			for (TestResult testResult : suiteResult.getTestResults()) {
				...
				writeTableData(
						decimalFormat.format(millisecondsToSeconds(duration)),
						"num");
		...
		if (testIndex >= 1) {
			...
			writeTableHeader(
					decimalFormat.format(millisecondsToSeconds(totalDuration)),
					"num");
		...
		if (!classResults.isEmpty()) {
			...
			for (ClassResult classResult : classResults) {
				...
				for (MethodResult methodResult : classResult.getMethodResults()) {
					...
					String methodName = Utils.escapeHtml(aResult.getMethod()
							.getMethodName());
					...
					for (ITestResult result : results) {
						...
						for (Object parameter : result.getParameters()) {
							...
							str = str
									+ Utils.escapeHtml(Utils.toString(
						...
						buffer.append("<td><a href=\"#m")
								...
								.append(decimalFormat
										.format(millisecondsToSeconds(duration)))
								.append("</td>").append("<td>" + str + "</td>")
								.append("<td>" + firstLine + "</td></tr>");
						...
						/*
						 * // Write the remaining scenarios for the method for (
						 * int i = 1; i < resultsCount; i++ ) {
						 * buffer.append("<tr class=\"").append(cssClass)
						 * .append("\">").append("<td><a href=\"#m")
						 * .append(scenarioIndex).append("\">") .append(
						 * methodName + "</a></td>" )
						 * .append("<td rowspan=\"1\">" + aResult.getName() +
						 * "</td>" ) .append("<td>"+str+"</td>")
						 * .append("<td>"+aResult
						 * .getMethod().getDescription()+"</td>")
						 * .append("<td>"+ firstLine+"</td></tr>");
						 * scenarioIndex++;
						 * 
						 * 
				...
				System.out.println("scenarioIndex " + scenarioIndex);
				System.out.println("scenarioClass " + scenariosPerClass);
				*/// Write the test results for the class
				...
				writer.print(scenarioIndex );
				writer.print("\">");
				writer.print(Utils.escapeHtml(classResult.getClassName().substring(classResult.getClassName().lastIndexOf(".") + 1)));
				writer.print("</td>");
				writer.print(buffer);
		...
		for (ClassResult classResult : classResults) {
			...
			for (MethodResult methodResult : classResult.getMethodResults()) {
				...
				String label = Utils.escapeHtml(className + "#"
						+ mResult.getMethod().getMethodName() + " ( "
						+ mResult.getName() + " )");
				for (ITestResult result : results) {
